...
class UseMemberCouponService implements UseMemberCouponUseCase {
    ...
    public UseMemberCouponResponse useMemberCoupon(UseMemberCouponCommand command) {
        MemberCoupon memberCoupon = findCouponOfMemberPort.findCouponOfMember(command.getMemberId(), command.getTicketId())
        ...
        return new UseMemberCouponResponse(useMemberCoupon.getId(), command.getTicketId(), useMemberCoupon.getUseDateTime());
    }
}
